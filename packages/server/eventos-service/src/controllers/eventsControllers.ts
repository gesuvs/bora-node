...
export class EventController {
  public async create(req: Request, res: Response): Promise<Response> {
    ...
    if (isNaN(price) || price === '') {
      return res.status(400).send({ err: 'Valor do evento invalido' });
    }

    if ((isFree === false && price === 0) || price < 0) {
      if (price < 0)
        return res.status(400).send({ err: 'Valor do evento invalido' });
      res.status(400).send({ err: 'Evento pago, deverá informar um valor' });
    }
    const startDayFormatted = parse(startDay, 'dd/MM/yyyy', new Date());
    const startEndFormatted = parse(startEnd, 'dd/MM/yyyy', new Date());
    const timeStartFormatted = parse(startTime, 'HH:mm', new Date());
    const timeEndFormatted = parse(endTime, 'HH:mm', new Date());
  ...
  public async findAll(
    ...
    const { page, keys } = req.query ?? undefined;
    const events: Event[] = await prisma.event.findMany({
      ...
      where: {
        name: {
          contains: String(keys) === 'undefined' ? undefined : String(keys),
        },
      },
    });
  ...
  public async findAllParticipante(req: Request, res: Response) {
    const { id } = req.params;
