...
const paymentServiceProxy = httpProxy(
  `http://${process.env.PAYMENT_SERVICE}:${process.env.PAY_PORT}`
);

const uploadServiceProxy = httpProxy(
  `http://${process.env.UPLOAD_SERVICE}:${process.env.UPLOAD_PORT}`
);
...
routes.post("/upload/*", (req, res, next) => {
  uploadServiceProxy(req, res, next);
});
